...
for tup in os.walk(path):
    ...
    if len(files) > 0:
        if 'angle' in fold:
            print(fold)
        if 'angle' in fold and '/0' in fold:
            fold1 = fold[:]
            fold2 = fold.replace('/0', '/10')
        elif 'angle' in fold and '/10' in fold:
            fold1 = fold[:]
            fold2 = fold.replace('/10', '/45')
        elif 'angle' in fold and '/45' in fold:
            fold2 = fold[:]
            fold1 = fold.replace('/45', '/0')
        else:
            continue
        try:
